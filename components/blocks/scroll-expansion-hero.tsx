...
const ScrollExpandMedia = ({
  ...
  // Mirrors of the latest values so the wheel/touch listeners can read them
  // without being re-bound on every scroll frame.
  const progressRef = useRef(0);
  const expandedRef = useRef(false);
  const touchStartRef = useRef(0);

  // Reset when the media changes — adjust during render (no setState-in-effect).
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      if (expandedRef.current && e.deltaY < 0 && window.scrollY <= 5) {
        ...
        e.preventDefault();
        updateProgress(progressRef.current + e.deltaY * 0.0009);
      }
    };
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      if (expandedRef.current && deltaY < -20 && window.scrollY <= 5) {
        ...
        updateProgress(progressRef.current + deltaY * scrollFactor);
