...
export interface HeroProps {
  /** Three sections: index 0 is the opening view, 1 & 2 are revealed on scroll. */
...
const DEFAULT_SECTIONS: HeroSection[] = [
  {
    title: "STUDIO",
    line1: "On la visió es troba amb la realitat,",
    line2: "donem forma al futur",
  },
  {
    title: "CRAFT",
    line1: "Més enllà dels límits de la imaginació,",
    line2: "hi ha un univers de possibilitats",
  },
  {
    title: "VISION",
    line1: "Entre el pensament i la creació,",
    line2: "trobem l'essència de la innovació",
  },
];

export const Component = ({ sections = DEFAULT_SECTIONS }: HeroProps) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Use the hero container's own height so the effect completes within the
      // hero, even when more content follows on the page.
      const heroHeight =
        containerRef.current?.offsetHeight ?? document.documentElement.scrollHeight;
      const maxScroll = Math.max(heroHeight - windowHeight, 1);
      ...
      if (refs.mountains.length && refs.locations) {
        refs.mountains.forEach((mountain: any, i: number) => {
          const speed = 1 + i * 0.9;
  ...
  const splitTitle = (text: string) => {
    // Split on explicit line breaks (\n) into lines, then words, then chars —
    // so a full name can be forced onto two lines while each char still animates.
    return text.split("\n").map((line, li) => (
  ...
  const scrollSections = sections.slice(1);
